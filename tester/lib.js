...
import {filter, first, switchMap, shareReplay, debounceTime, startWith, share, take, catchError, tap, timeout, retry, map} from "rxjs/operators";
...
export const appsyncRealtime = (APIURL, connectionRetryConfig = {}) => {
	...
	const appSyncWebSocket = (getAuthorizationHeaders, opened, subscriptionRetryConfig = {}) => (query, variables) => new Observable((observer) => {
		...
		if (subscribers === 0) {
			currentWs = new Observable((observer) => {
				const subscription = new Subscription();
				Promise.resolve(getAuthorizationHeaders({connect: true, data: {}})).then((authHeaders) => {
		...
		subscribers++;
		const subscription = currentWs.pipe(
